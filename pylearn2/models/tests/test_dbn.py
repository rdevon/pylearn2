...
class Test_DBN_Basics(object):
    ...
    def test_deeper_dbn(self, dims = [100, 150, 200, 250], batch_sizes=[5, 13, 23]):
        dbn = Test_DBN_Basics.make_deeper_dbn(dims=dims, batch_sizes=batch_sizes)
        assert dbn is not None

    def test_shared_variables(self):
        dbn = Test_DBN_Basics.make_deeper_dbn()
        for rbm, mlp_layer in zip(dbn.rbms, dbn.mlp.layers):
            ...
            assert hidden_layer.b == mlp_layer.layer_content.b
            assert hidden_params == mlp_layer_params,\
                                  "Params not the same: hidden:\n%r\nmlp layer:\n%r" % (hidden_params, mlp_layer_params)
    ...
    def test_feed_forward(self):
        ...
        X = sharedX(rng.randn(dbn.rbms[0].batch_size, dbn.rbms[0].visible_layer.nvis))
        Y_mf = dbn.feed_forward(X, method="MF")[-1]
        Y_s = dbn.feed_forward(X, method="SAMPLING", theano_rng=theano_rng)[-1]
        assert np.allclose(Y_mf.eval().mean(axis=0), Y_s.eval().mean(axis=0),rtol=1e-1,atol=1e-1)

    def test_generate(self, batch_size=1):
        rng = np.random.RandomState([2014,10,19])
        dbn = Test_DBN_Basics.make_deeper_dbn()
        Y = sharedX(rng.randn(batch_size, dbn.top_rbm.hidden_layer.detector_layer_dim))
...
class Test_DBN_as_RBM(object):
    def test_CD(self):
        ...
        pos_grads, up_pos = Test_CD.check_rbm_pos_phase(dbn.top_rbm, cost, X_hat, tol=0.59)
        neg_grads, up_neg = Test_CD.check_rbm_neg_phase(dbn.top_rbm, cost, X_hat, theano_rng=theano_rng, tol=0.6)
        for pos in pos_grads:
            assert np.all(pos_grads[pos].shape.eval() == neg_grads[pos].shape.eval()),\
                "Gradient shapes do not match for %r (%r vs %r)" % (pos,
                                                                                                      pos_grads[pos].shape.eval(),
        ...
        updates, layer_to_chains = dbn.get_sampling_updates(layer_to_chains,
                                                              theano_rng,
                                                              num_steps=1,
        ...
        for grad in neg_phase_grads:
            ...
            assert np.all(pos_grads[grad].shape.eval() == neg_grads[grad].shape.eval()),\
                "%r grad shapes do not match (%r vs %r)"\
                %(grad, pos_grads[grad].shape.eval(), neg_grads[grad].shape.eval())
